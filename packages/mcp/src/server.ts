...
server 会自动读取本地文件、上传到七牛、把 URL 改回 markdown 再提交。`,
  ...
  server.tool(
    ...
    "重命名标签。传入标签 ID 和新名称。",
    {
      ...
      slug: z.string().optional().describe("自定义 slug（可选，默认从名称生成）"),
    },
    async ({ id, name, slug }) => {
      try {
        const tag = await sdk.tags.update(id, { name, slug });
        return json({ id: tag.id, name: tag.name, slug: tag.slug });
      } catch (err) {
        return errorContent((err as Error).message);
      }
    },
  ...
  server.tool(
    "merge_tags",
    "合并标签：将源标签的所有文章关联迁移到目标标签，然后删除源标签。用于清理重复标签。",
    {
      source_id: z.number().int().describe("源标签 ID（将被删除）"),
      target_id: z.number().int().describe("目标标签 ID（保留）"),
    },
    async ({ source_id, target_id }) => {
      try {
        const result = await sdk.tags.merge(source_id, target_id);
        return json(result);
      } catch (err) {
        return errorContent((err as Error).message);
      }
