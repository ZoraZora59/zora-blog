...
- **底部**：Footer（GitHub / LinkedIn / Instagram 链接 + RSS 订阅入口）
...
- **内容区块**：
  ...
  - **社交链接**：GitHub / LinkedIn / Instagram / 邮箱等，图标按钮
  - **联系方式**：邮箱（可选展示）
...
- **路由**：`/feed.xml`
- **格式**：标准 RSS 2.0 / Atom Feed
- **内容**：最新 20 篇已发布文章，包含标题、摘要、发布时间、文章链接
- **自动发现**：HTML `<head>` 中添加 `<link rel="alternate" type="application/rss+xml">` 标签
- **入口**：Footer 中的 RSS 图标链接
- **技术实现**：后端动态生成 XML，缓存 10 分钟
...
| GET | /feed.xml | RSS 订阅 Feed（最新 20 篇文章） |
...
| M6 | 扩展功能：关于页、搜索（全文检索）、RSS 订阅、Dark Mode | M2, M3 |
